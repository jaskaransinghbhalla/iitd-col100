...
def valid_sudoku(sudoku:List[List[int]])-> bool:
	...
	for i in range(1,10):
		if(valid_list(get_row(sudoku,i)) == False):
			return False
	for j in range(1,10):
		if(valid_list(get_column(sudoku,j)) == False):
			return False
	for k in range(1,10):
		if(valid_list(get_block(sudoku,k)) == False):
			return False	
	return True
...
def sudoku_solver(sudoku: List[List[int]]) -> Tuple[bool, List[List[int]]]:
	...
	return(valid_sudoku(sudoku), sudoku)
	# to complete this function, you ma y define any number of helper functions.
...
# def in_lab_component(sudoku: List[List[int]]):
	...
	# print(valid_list(get_row(sudoku,1)))
