...
def valid_list2(lst: List[int])-> bool:
		ans = True
		for i in range(0,len(lst)):
			if lst[i]==0:
				if lst.count(0)>1:
...
def valid_sudoku(sudoku:List[List[int]])-> bool:
	...
	final=True
	for i in range(1,10):
		if(valid_list2(get_row(sudoku,i)) == False):
			final = False
	for j in range(1,10):
		if(valid_list2(get_column(sudoku,j)) == False):
			final = False
	for k in range(1,10):
		if(valid_list2(get_block(sudoku,k)) == False):
			final = False	
	return final
...
def sudoku_solver(sudoku: List[List[int]]) -> Tuple[bool, List[List[int]]]:
	...
	for each in candidates :
		sudoku = make_move(sudoku,find_first_unassigned_position(sudoku),each)
