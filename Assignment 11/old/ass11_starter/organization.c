...
int height(struct Employee* p){
    if(p == NULL){
        return 0;
    }
    return (1 + max(height(p->subordinate_1),height(p->subordinate_2)));
...
int Diameter_helper(struct Employee *p){
    ...
    int o1 = height(p->subordinate_1) + height(p->subordinate_2);
